...
@RestController
@Slf4j
public class ChangeStateController2 {
    private final StateMachine<String, String> stateMachine;

    public ChangeStateController2(StateMachine<String, String> stateMachine) {
        this.stateMachine = stateMachine;
    }


    @GetMapping("change2/{event}")
    public String changeState2(@PathVariable @Nullable String event) {
        if (event.isEmpty()) {
            return "empty event";
        }
